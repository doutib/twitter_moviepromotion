...
da = pd.read_csv('summary.csv')
df = da.rename(index=da['handle_name'])
screen_names = list(df['handle_name'])
screen_names.remove("7") #delte for 007
...
sale = [df.loc[movie]['sales'] for movie in screen_names]
...
del movies['users'][77]  #delete for 007
order = [screen_names.index(movies['users'][i]['screen_name']) for i in range(len(movies['users']))]

followers = range(88)
for i in range(len(movies['users'])):
	followers[order[i]] = movies['users'][i]['followers_count']
